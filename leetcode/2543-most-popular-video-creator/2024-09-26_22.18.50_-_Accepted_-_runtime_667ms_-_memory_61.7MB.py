class Solution:
    def mostPopularCreator(self, creators: List[str], ids: List[str], views: List[int]) -> List[List[str]]:
        ...
        for key, value in count.items():
            max_views = 0
            total_views = 0
            id_with_max_views = value[0]
            # for each creator get max (id, views) pair
            for i in range(0, len(value), 2):
                if value[i+1] > max_views:
                    id_with_max_views, max_views = value[i], value[i+1]
                elif value[i+1] == max_views: # get lexiocographically smaller id if views are the same
                    if value[i] < id_with_max_views:
                        id_with_max_views = value[i] 
